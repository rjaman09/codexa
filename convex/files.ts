...
export const renameFile = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        // check if file with the newName already exists in same folder
        ...
        if (existing) {
            throw new Error(`A ${file.type} with same name already exists in this location`);
        }
...
export const deleteFile = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const deleteRecursive = async (fileId: Id<"files">) => {
            ...
            // if it's folder, delete all children
            if (item.type === "folder") {
